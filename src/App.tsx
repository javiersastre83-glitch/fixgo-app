...
export default function App({ session }) {
    ...
    return(
      <div style={s.root}>
        ...
        <div style={{flex:1,overflowY:"auto"}}>
          ...
          {(()=>{const miembroDetalle=detalle.responsable_usuario_id?equipoObra.find(m=>m.uid===detalle.responsable_usuario_id):null;const filaResponsable=<div key="Responsable" style={{display:"flex",alignItems:"center",gap:12,padding:"13px 0",borderBottom:"1px solid #E0E0E5"}}><span style={{width:24,display:"flex",justifyContent:"center",color:"#8E8E93"}}><User size={20}/></span><span style={{fontSize:13,color:"#6B6B70",fontWeight:600,width:90}}>Responsable</span><span style={{flex:1,display:"flex",alignItems:"center",gap:8,justifyContent:"flex-end"}}>{miembroDetalle&&<span style={{width:24,height:24,borderRadius:"50%",background:colorPastelDe(miembroDetalle.uid),display:"inline-flex",alignItems:"center",justifyContent:"center",fontSize:11,fontWeight:800,color:"#fff",flexShrink:0}}>{miembroDetalle.nombre?miembroDetalle.nombre[0].toUpperCase():""}</span>}<span style={{fontSize:16,color:"#000",fontWeight:700}}>{miembroDetalle?miembroDetalle.nombre:detalle.responsable}</span></span></div>;return[filaResponsable,...[[<MapPin size={20}/>,"Sector",detalle.sector],detalle.fechaLimite?[<Calendar size={20}/>,"Fecha límite",formatFecha(detalle.fechaLimite)]:null,[<Calendar size={20}/>,"Cargada",detalle.fecha?formatFecha(detalle.fecha):"—"]].filter(Boolean).map(([ic,lb,vl])=>(<div key={lb} style={{display:"flex",alignItems:"center",gap:12,padding:"13px 0",borderBottom:"1px solid #E0E0E5"}}><span style={{width:24,display:"flex",justifyContent:"center",color:"#8E8E93"}}>{ic}</span><span style={{fontSize:13,color:"#6B6B70",fontWeight:600,width:90}}>{lb}</span><span style={{flex:1,fontSize:16,color:"#000",fontWeight:700,textAlign:"right"}}>{vl}</span></div>))];})()}
  ...
  return(
    <div style={{...s.root,position:"relative"}}>
      ...
      {menuContextual&&<div style={s.overlay} onClick={()=>setMenuContextual(null)}><div style={s.modal} onClick={e=>e.stopPropagation()}><p style={{margin:"0 0 16px",fontSize:17,fontWeight:700}}>Opciones</p>{(()=>{const nov=novedades.find(n=>n.id===menuContextual.novId);const puedeReabrirOResolver=nov&&(!nov.resuelta||nov.autorId===miId||puedeGestionar);return puedeReabrirOResolver?<button style={{...s.btnPrincipal,background:"#F2F2F7",color:"#1C1C1E",marginBottom:10}} onClick={()=>{resolver(menuContextual.novId);setMenuContextual(null);}}>{nov?.resuelta?"↩ Reabrir":"✅ Marcar como resuelto"}</button>:null;})()}<button style={{...s.btnPrincipal,background:"#F2F2F7",color:"#1C1C1E",marginBottom:10}} onClick={()=>{setAsignacionRapida(menuContextual.novId);setMenuContextual(null);}}><span style={{display:"flex",alignItems:"center",gap:6}}><User size={15}/>Asignar a alguien del equipo</span></button><button style={{...s.btnPrincipal,background:"#FF3B3010",color:"#FF3B30",marginBottom:10}} onClick={()=>{setConfirmarEliminar(menuContextual.novId);setMenuContextual(null);}}><span style={{display:"flex",alignItems:"center",gap:6}}><Trash2 size={15}/>Eliminar</span></button><button style={{...s.btnPrincipal,background:"#F2F2F7",color:"#8E8E93"}} onClick={()=>setMenuContextual(null)}>Cancelar</button></div></div>}
      {asignacionRapida&&(()=>{const nov=novedades.find(n=>n.id===asignacionRapida);if(!nov)return null;return(
        <div style={s.overlay} onClick={()=>setAsignacionRapida(null)}><div style={{...s.modal,maxHeight:"75vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
          ...
          <div style={{overflowY:"auto"}}>
            <TiraResponsables value={nov.responsable} usuarioId={nov.responsable_usuario_id} equipo={equipoObra} onChange={({responsable,usuarioId})=>asignarRapido(nov.id,{responsable,usuarioId})} />
          </div>
